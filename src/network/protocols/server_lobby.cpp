...
ServerLobby::ServerLobby() : LobbyProtocol()
{
    ...
    ServerConfig::m_super_tournament_qualification = false;
    ...
    ServerConfig::m_rank_soccer = true;
...
void ServerLobby::changeTeam(Event* event)
{
    if (!ServerConfig::m_team_choosing ||
        !RaceManager::get()->teamEnabled() || ServerConfig::m_rank_soccer)
        return;
...
void ServerLobby::startSelection(const Event *event)
{
	...
	std::string msg="";
        for (auto peer_rdy : m_peers_ready)
        {
            ...
            if (canRace(peer) && !peer->alwaysSpectate() && !peer->isWaitingForGame() && peer->isValidated())
            {
                for (auto player : peer->getPlayerProfiles())
                {
                    std::string username = StringUtils::wideToUtf8(player->getName());
...
void ServerLobby::finishedLoadingWorldClient(Event *event)
{
    ...
    bool super= ServerConfig::m_super_tournament && ServerConfig::m_count_supertournament_game;
    if ( super || ServerConfig::m_rank_soccer)
    {
        std::string username = StringUtils::wideToUtf8(peer->getPlayerProfiles()[0]->getName());
	std::string redname;
	std::string bluename;
        if(super) redname=ServerConfig::m_red_team_name;
        if(super) bluename=ServerConfig::m_blue_team_name;
                                      ...
                                      std::shared_ptr<STKPeer> peer)
{
	...
	if (ServerConfig::m_super_tournament || ServerConfig::m_super_tournament_qualification)
    {
        ...
        // Following commands are SuperTournament only
    ...
    if (ServerConfig::m_super_tournament_qualification)
    {
        ...
        else if (argv[0] == "stop")
        {
            ...
            SoccerWorld *sw = dynamic_cast<SoccerWorld*>(w);
            sw->stop();
            m_super_tourn_quali.gameState.gameStoppedAt(sw->getTime());
        ...
        else if (argv[0] == "go" || argv[0] == "play" || argv[0] == "resume")
        {
            ...
            SoccerWorld *sw = dynamic_cast<SoccerWorld*>(w);
            sw->resume();
            m_super_tourn_quali.gameState.gameResumedAt(sw->getTime());
        ...
        else if (argv[0] == "lobby")
        {
            ...
            SoccerWorld *sw = dynamic_cast<SoccerWorld*>(w);
            m_super_tourn_quali.gameState.gameStoppedAt(sw->getTime());
            m_super_tourn_quali.gameState.initGoals(sw->getScore(KART_TEAM_RED), sw->getScore(KART_TEAM_BLUE));
        ...
        else if (argv[0] == "init")
        {
            if (argv.size() != 4)
            {
                std::string msg = "Format: /init red_goals blue_goals minutes";
                sendStringToPeer(msg, peer);
                return;
            }
            int red_goals = 0, blue_goals = 0, minutes = 0;
            ...
            for (int i = 1; i < 4; i++)
            {
                ...
                {
                    if (i == 1) red_goals = std::stoi(param);
                    else if (i == 2) blue_goals = std::stoi(param);
                    else if (i == 3) minutes = std::stoi(param);
                }
                else
                {
                    std::string msg = "Invalid parameters. Format: /init red_goals blue_goals minutes";
                    sendStringToPeer(msg, peer);
                    return;
                }
            }
